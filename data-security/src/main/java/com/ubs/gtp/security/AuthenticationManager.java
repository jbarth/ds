...
public class AuthenticationManager {
    ...
    public TokenWithId authenticateClient(String username, String password, String pin) throws AuthenticationException {
		...
		} else if(clientLogin.getFailedAttempts() > 2) {
			hibernateSession.getTransaction().commit();
			throw new AuthenticationException("User locked.");
		} else if (!valueAndValueHashMatches(password, clientLogin)) {
			clientLogin.setFailedAttempts(clientLogin.getFailedAttempts() + 1);
			hibernateSession.save(clientLogin);
			hibernateSession.getTransaction().commit();
    ...
    public TokenWithId authenticateAdvisor(String username, String password) throws AuthenticationException {
        ...
        if (advisorLogin == null) {
            throw new AuthenticationException("User [" + username + "] does not exist.");
        } else if (!valueAndValueHashMatches(password, advisorLogin)) {
            throw new AuthenticationException("Password does not match.");
        }
        // create a session if the sessionManager exists
        if (sessionManager != null) {
    ...
    public boolean valueAndValueHashMatches(String loginPassword, UsernamePasswordHashLogin usernamePasswordHashLogin) {
