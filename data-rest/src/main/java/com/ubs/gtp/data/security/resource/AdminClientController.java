...
public class AdminClientController {
	...
	@ResponseBody
	public UsernameWithId addClient(@RequestBody Client request, final HttpServletResponse response) {
		...
		clientLoginData.setId(clientData.getClientId());
...
//		clientLoginData.setClient(clientData);
		...
		if (!checkClient.isPresent() || checkClient.get().getName().equals(request.getName())) {
			...
			usernameWithId.setUsername(clientLoginData.getUsername());
			usernameWithId.setId(clientLoginData.getId());
			return usernameWithId;
		} else {
